  ...
  const transaction = db.transaction(() => {
    ...
    if (saleItems && saleItems.length > 0) {
      const stmtItem = db.prepare(`
        INSERT OR IGNORE INTO sale_items (id, sale_id, product_id, product_name, qty, buy_price, sell_price)
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `);
      for (const item of saleItems) {
        stmtItem.run(item.id, item.saleId, item.productId, item.productName || item.name, item.qty, item.buyPrice, item.sellPrice);
      }
    }
  });
