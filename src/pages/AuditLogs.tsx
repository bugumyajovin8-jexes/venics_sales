...
export default function AuditLogs() {
  ...
  const monthKeys = useMemo(() => {
    return Object.keys(groupedLogs).sort((a, b) => {
      const dateA = new Date(groupedLogs[a][0].created_at);
      const dateB = new Date(groupedLogs[b][0].created_at);
      ...
      <div className="flex-1 overflow-y-auto space-y-4 pb-10">
        {monthKeys.length > 0 ? (
          monthKeys.map((monthKey) => {
            ...
            return (
              <div key={monthKey} className="flex flex-col space-y-2">
                {/* Month Header */}
                <button
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className={`text-[10px] sm:text-xs font-medium ${isCurrentMonth ? 'text-indigo-100' : 'text-gray-400'}`}>
                        Mabadiliko {monthLogs.length} yaliyorekodiwa
                      </p>
                    </div>
                  </div>
