...
const QtyControl = ({ product, cartItem, updateQty, removeFromCart, showToast }: any) => {
  ...
  return (
    <div className="flex items-center bg-blue-50 rounded-lg p-0.5 w-full justify-between">
      ...
      <input
        ...
        onBlur={handleBlur}
        className="w-10 text-center text-[11px] font-black text-blue-700 mx-1 bg-transparent border-none focus:ring-0 p-0 m-0"
      />
      <button 
        onClick={(e) => {
          e.stopPropagation();
...
const CartItemRowItem = ({ item, currency }: { item: any, currency: string }) => {
  const removeFromCart = useStore(state => state.removeFromCart);
  return (
    <div className="px-2 mb-2">
      <div className="bg-white border border-gray-100 rounded-2xl p-3 flex items-center justify-between shadow-sm">
...
export default function Kikapu() {
  ...
  const ProductSelectionRow = useCallback(({ index, style }: RowComponentProps) => {
    const productsInRow = [
  ...
  }, [filteredProducts, cart, currency, addToCart, removeFromCart, updateQty, showToast]);
  ...
  const handleCompleteSale = async (paymentMethod: 'cash' | 'credit') => {
    ...
    try {
      ...
      await db.transaction('rw', db.products, db.sales, db.saleItems, async () => {
        ...
        TelemetryService.trackSale(sale.payment_method, sale.total_amount, cart.length);
        
        if (isCreditSale) {
          const nameWords = customerName.trim().split(/\s+/).length;
          const totalCartAmt = cartTotal();
      ...
      if (settings?.autoInvoice) {
        try {
          ...
          doc.text(`Jumla Kuu: ${formatCurrency(cartTotal(), currency)}`, 14, finalY);
          ...
          {isDiscountMode ? (
            cart.length > 0 ? (
              <div className="h-full overflow-y-auto w-full pb-20 scrollbar-hide">
                {cart.map((item) => (
                  <CartItemRowItem key={item.id} item={item} currency={currency} />
                ))}
              </div>
            ) : (
            ...
            <div className="flex items-center justify-between px-4 py-1">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-[9px] text-gray-400 font-bold uppercase tracking-widest">Mauzo</p>
                </div>
              </div>
              <button 
                onClick={() => {
